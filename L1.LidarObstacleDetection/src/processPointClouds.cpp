...
template<typename PointT>
std::vector<typename pcl::PointCloud<PointT>::Ptr> ProcessPointClouds<PointT>::ClusteringRubric(typename pcl::PointCloud<PointT>::Ptr cloud, float clusterTolerance, int minSize, int maxSize)
{
    auto startTime = std::chrono::steady_clock::now();

    std::vector<typename pcl::PointCloud<PointT>::Ptr> clusters;

    // Create implemented KD-tree
    ...
    for (int i=0; i<cloud->points.size(); i++)
    {
        ...
        std::vector<float> point_vector{point.x, point.y, point.z};
        tree->insert(point_vector, i); 
        points.push_back(point_vector);
    }

    // Perfrom cluster with implemented method
